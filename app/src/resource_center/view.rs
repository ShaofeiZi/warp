...
impl ResourceCenterFooterItem {
    pub fn ui_label(&self, app: &AppContext) -> String {
        match self {
            ResourceCenterFooterItem::Docs => I18n::as_ref(app).tr(I18nKey::CommonDocumentation),
            ResourceCenterFooterItem::Slack => I18n::as_ref(app).tr(I18nKey::CommonSlack),
            ResourceCenterFooterItem::Feedback => I18n::as_ref(app).tr(I18nKey::CommonFeedback),
        }
    }
    ...
    fn render_header_contents(
        ...
        let header_text = match current_page {
            Some(ResourceCenterPage::Keybindings) => {
                I18n::as_ref(app).tr(I18nKey::ResourceCenterKeyboardShortcuts)
            }
            _ => {
                if FeatureFlag::AvatarInTabBar.is_enabled() {
                    String::new()
                } else {
                    I18n::as_ref(app).tr(I18nKey::ResourceCenterWarpEssentials)
                }
            }
        };
